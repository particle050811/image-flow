...
/** 模型切换涉及的六个 config 键：工作台用 model 组，编辑页用 editModel 组。
 *  按值类型收窄——provider/model/size/ratio 必为字符串字段、memory/params 必为记录字段，传错组别即编译期报错 */
...
/** 记忆值在支持列表内则恢复，否则沿用当前值（仍受支持时），再不行取首项 */
function pickRemembered(remembered: string | undefined, supported: readonly string[], current: string): string {
	return remembered && supported.includes(remembered)
		? remembered
 ...
 * 否则沿用当前值（新模型也支持时），仍不行取首项；自定义参数逐键校验记忆值仍在可选项内，否则用默认值。
...
export function switchModelParams(
	...
	return {
		snapshot: {
			imageSize: pickRemembered(remembered?.imageSize, target.imageSizes, oldSnapshot.imageSize),
			aspectRatio: pickRemembered(remembered?.aspectRatio, target.aspectRatios, oldSnapshot.aspectRatio),
...
/**
 * 工作台与编辑页共用的「模型切换」接线：取出当前 (渠道, 模型) 对应的模型项（含档位/比例/自定义参数），
 * 并给出切模型处理器（记下旧档→按记忆/兜底取新档→比例越界回退首项→播种参数默认值→一次性批量写回）。
...
export function modelSizeControl(
	...
	const changeModel = (qualified: string) => {
		...
		const { snapshot, memory } = switchModelParams(
			...
			{ aspectRatio: config[keys.ratio], imageSize: config[keys.size], params: config[keys.params] },
